...
static bool mount_partition(struct list_elem* pelem, int arg)
{
	...
	if (!strcmp(part->name, part_name)) {	// 若传的分区的名字和arg所指的字符串相等
		cur_part = part;
		struct disk* hd = cur_part->mydisk;
...
static void partition_format(struct disk* hd, struct partition* part) {
	...
	uint32_t inode_bitmap_sects = DIV_ROUND_UP(MAX_FILES_PER_PART, BITS_PER_SECTOR);	// inode位图所需扇区数
	...
	block_bitmap_sects = DIV_ROUND_UP(free_sects, BITS_PER_SECTOR);		// 块位图自身所用扇区数

	/* block_bitmap_bit_len 是位图中位的长度，也是可用块的数量 */
	uint32_t block_bitmap_bit_len = free_sects - block_bitmap_sects;	// 减去块位图自身所用扇区
...
/******************************* 2 将块位图初始化并写入sb.block_bitmap_lba ******************************/
	...
	while (bit_idx <= block_bitmap_last_bit) {
		buf[block_bitmap_last_byte] &= ~(1 << bit_idx++);	// 将1左移,从0到last_bit依次置0
	}
	ide_write(hd, sb.block_bitmap_lba, buf, sb.block_bitmap_sects);

/******************************* 3 将 inode 位图初始化并写入sb.inode_bitmap_lba ******************************/
	/* 先清空缓冲区*/
	memset(buf, 0, buf_size);
	buf[0] |= 0x01;		// 第0个inode分给了根目录
	/* 由于 inode_table 中共4096个inode， 
     * 位图inode_bitmap正好占用1扇区，       
	 * 即inode_bitmap_sects 等于1， 
     * 所以位图中的位全都代表inode_table中的inode， 
	 * 无需再像block_bitmap那样单独处理最后一扇区的剩余部分， 
	 * inode_bitmap 所在的扇区中没有多余的无效位 */ 
	ide_write(hd, sb.inode_bitmap_lba, buf, sb.inode_bitmap_sects);
	
/*******************************  4 将 inode 数组初始化并写入sb.inode_table_lba  ******************************/
	/* 准备写inode_table中的第0项,即根目录所在的inode */ 
	memset(buf, 0, buf_size);
	struct inode* i = (struct inode*)buf;
	i->i_size = sb.dir_entry_size * 2;		// .和.. 
	i->i_no = 0;							// 根目录占 inode数组中第0个inode
...
static int search_file(const char* path_name, struct path_search_record* searched_record)
{
	...
	uint32_t path_len = strlen(path_name);
	// 保证 pathname 至少是这样的路径/x，且小于最大长度
	ASSERT(path_name[0] == '/' && path_len > 1 && path_len < MAX_PATH_LEN);
	char* sub_path = (char*)path_name;
	...
	while (name[0]) {		// 若第一个字符就是结束符，结束循环
		...
		// 记录已存在的父目录
		strcat(searched_record->searched_path, "/");	// 这里要传字符串常量，不能传字符asci码
		...
		if (search_dir_entry(cur_part, parent_dir, name, &dir_e)) {
			...
			if (dir_e.f_type == FT_DIRECTORY) {		// 如果被打开的是目录
				// 保存当前目录的父目录
				...
				searched_record->parent_dir = parent_dir;
				continue;
			} else if (dir_e.f_type == FT_REGULAR) {	// 若是普通文件
...
int32_t sys_open(const char* path_name, uint8_t flags)
{
	...
	uint32_t path_name_depth = path_depth_cnt((char*)path_name);
	/* 先检查文件是否存在 */
	int inode_no = search_file(path_name, &searched_record);
	bool found = inode_no != -1 ? true : false;

	if (searched_record.file_type == FT_DIRECTORY) {
		printk("can't open directory by open(), please use opendir()\n");
		dir_close(searched_record.parent_dir);
		return -1;
	}
	...
	if (path_name_depth != path_searched_depth) {	 // 说明并没有访问到全部的路径，某个中间目录是不存在的
		printk("can't access %s: Not a directory, subpath %s is not exit\n", path_name, searched_record.searched_path);
		dir_close(searched_record.parent_dir);
		return -1;
	}
	...
	if (!found && !(flags & O_CREATE)) {
		...
		dir_close(searched_record.parent_dir);
		return -1;
	} else if (found && (flags & O_CREATE)) {	// 若要创建的文件已存在
		printk("%s has already exit!\n", path_name);
		dir_close(searched_record.parent_dir);
		return -1;
	}
	...
	switch (flags & O_CREATE) {
		case O_CREATE:	// 没找到文件并创建文件
			printk("crating file %s\n", path_name);
			fd = file_create(searched_record.parent_dir, (strrchr(searched_record.searched_path, '/') + 1), flags);
			dir_close(searched_record.parent_dir);
			break;
		default:	// 找到文件并打开
			// 其余情况均为打开已存在文件O_RDONLY,O_WRONLY,O_RDWR
			printk("%s exits and open it\n", path_name);
			fd = file_open(inode_no, flags);
	}

	/* 此 fd 是指任务pcb->fd_table数组中的元素下标，并不是指全局file_table中的下标 */ 
	return fd;	
}
...
int32_t sys_close(int32_t fd)
{
	...
	if (fd > 2) {
		...
		if (is_pipe(fd)) {
			if (--file_table[global_fd].fd_pos == 0) {	// 若关闭一次后打开次数为0就回收管道
...
{
	if (fd < 0) {
		printk("in sys_write: fd error\n");
		return -1;
	}
	if (fd == stdout_no) {
		if (is_pipe(fd)) {
			/* 标准输出有可能被重定向为管道缓冲区，因此要判断 */
			return pipe_write(fd, (void*)buf, count);
		} else {
			char tmp_buf[1024] = {0};
		...
		if ((wr_file->fd_flag & O_WRONLY) || (wr_file->fd_flag & O_RDWR)) {
			...
			console_put_str("in sys_write: not allowed to write file without flag O_RDWR or O_WRONLY\n", 0x07);
			return -1;
		}
	}
}
...
/* SEEK_SET 新的读写位置是相对于文件开头再增加offset个位移量 */
		case SEEK_SET:
			new_pos = offset;
			break;
/* SEEK_CUR 新的读写位置是相对于当前的位置增加offset个位移量,offset 可正可负*/
		case SEEK_CUR:
...
{
	ASSERT(strlen(path_name) <= MAX_PATH_LEN);
	
	/* 先检查待删除的文件是否存在 */
	struct path_search_record search_record;
	memset(&search_record, 0, sizeof(struct path_search_record));
	
	int inode_no = search_file(path_name, &search_record);
	ASSERT(inode_no != 0);
	if (inode_no == -1) {
		printk("sys_unlink: not found %s\n", path_name);
		dir_close(search_record.parent_dir);
		return -1;
	}

	if (search_record.file_type == FT_DIRECTORY) {
		printk("can't delete a directionary by unlink(), use rmdir() to instead\n");
		dir_close(search_record.parent_dir);
		return -1;
	}
	...
	if (file_idx < MAX_FILES_OPEN) {	// 文件在file_table中,文件被打开
		dir_close(search_record.parent_dir);
	...
	if (io_buf == NULL) {
		dir_close(search_record.parent_dir);
		printk("in sys_unlink: sys_malloc for io_buf failed\n");
		return -1;
	}

	struct dir* parent_dir = search_record.parent_dir;
	delete_dir_entry(cur_part, parent_dir, inode_no, io_buf);
	inode_realse(cur_part, inode_no);
	sys_free(io_buf);
	dir_close(search_record.parent_dir);
...
int32_t sys_mkdir(const char* path_name) 
{
	...
	void* io_buf = sys_malloc(SECTOR_SIZE * 2);
	if (io_buf == NULL) {
		printk("in sys_mkdir: sys_malloc for io_buf failed\n");
		return -1;
	}
	...
	memset(&searched_record, 0, sizeof(struct path_search_record));
	int inode_no = -1;
	inode_no = search_file(path_name, &searched_record);
	if (inode_no != -1) {	// 如果有同名目录或文件
		printk("sys_mkdir: file or directionary %s exists\n", path_name);
		rollback_step = 1;
		goto rollback;
	} else {
		...
		uint32_t path_searched_depth = path_depth_cnt(searched_record.searched_path);
		/* 先判断是否把pathname的各层目录都访问到了，即是否在某个中间目录就失败了 */
		if (path_name_depth != path_searched_depth) {	// 某个中间目录不存在
			printk("sys_mkdir: can not access%s: Not a directory, subpath %s is't exits\n", path_name, searched_record.searched_path);
			rollback_step = 1;
			goto rollback;
		}
	}

	// 中间目录存在,且没有目录或文件与要创建的目录重名
	struct dir* parent_dir = searched_record.parent_dir;
	// 目录名称后可能会有字符'/', 所以最好直接用searched_record.searched_path，无'/', 解析的时候不会留下多余的'/'
	char* dirname = strrchr(searched_record.searched_path, '/') + 1;	// 要建立的目录名的起点
	...
	// 将目录项.和..写回硬盘
	ide_write(cur_part->mydisk, new_dir_inode.i_sectors[0], io_buf, 1);
...
rollback:
	switch (rollback_step) {
		case 2:		// 块分配失败或目录项写入失败, 需要将之前位图中分配的inode_no也要恢复
			bitmap_set(&cur_part->inode_bitmap, inode_no, 0);	// 此时还未写入内存
		case 1:
			/* 关闭所创建目录的父目录 */
			dir_close(searched_record.parent_dir);
			break;
...
struct dir* sys_opendir(const char* name)
{
	...
	if (name[0] == '/' && (name[1] == '.' || name[1] == 0)) {	// 这里书上有误
		return &root_dir;
	}
...
int32_t sys_rmdir(const char* path_name)
{
	...
	if (inode_no == -1) {
		...
		if (searched_record.file_type == FT_REGULAR) {
			...
			struct dir* dir2del = dir_open(cur_part, inode_no);
			if (!dir_is_empty(dir2del)) {
				printk("dir %s is not empty, it is not allowed to delete a nonempty directory!\n", path_name);
			} else {
				if (!dir_remove(searched_record.parent_dir, dir2del)) {
					retval = 0;
				}
			}
			dir_close(dir2del);
...
static uint32_t get_parent_dir_inode_no(uint32_t child_inode_no, void* io_buf)
{
	...
	ide_read(cur_part->mydisk, block_lba, io_buf, 1);
...
/* 在inode编号为p_inode_nr的目录中查找inode编号为 c_inode_nr 的子目录的名字，将名字存入缓冲区path，成功返回0，失败返-1 */
static int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr, char* path, void* io_buf)
{
	struct inode* p_inode = inode_open(cur_part, p_inode_nr);
	uint32_t all_blocks[140] = {0}, block_idx = 0, block_cnt = 12;

	// 将父目录的所有块地址读进all_blocks
	while (block_idx < 12) {
		all_blocks[block_idx] = p_inode->i_sectors[block_idx];
		block_idx++;
	}
	if (p_inode->i_sectors[12] != 0) {
		ide_read(cur_part->mydisk, p_inode->i_sectors[12], all_blocks + 12, 1);
		block_cnt = 140;
	}
	inode_close(p_inode);
...
/* 把当前工作目录绝对路径写入buf，size是buf的大小。当buf为NULL时，由操作系统分配存储工作路径的空间并返回地址，失败则返回NULL */
char* sys_getcwd(char* buf, uint32_t size)
{
	/* 确保 buf不为空，若用户进程提供的buf为NULL，系统调用getcwd中要为用户进程通过malloc分配内存 */
	ASSERT(buf != NULL);
	...
	while (child_inode_nr) {
		parent_inode_nr = get_parent_dir_inode_no(child_inode_nr, io_buf);
	...
	/* 至此full_path_reverse中的路径是反着的，即子目录在前（左），父目录在后（右），现将full_path_reverse 中的路径反置 */
	char* last_slash;	// 用于记录字符串中最后一个斜杠地址
	while ((last_slash = strrchr(full_path_reverse, '/'))) {
		uint32_t len = strlen(buf);
		strcpy((buf + len), last_slash);	// 跨过现有的字符串
		// 在 full_path_reverse 中添加结束字符，作为下一次执行strcpy中last_slash的边界
		*last_slash = 0;
	}
	sys_free(io_buf);
	return buf;
...
void filesys_init()
{
	...
	while (channel_no < channel_cnt) {
		dev_no = 0;
		while (dev_no < 2) {
			if (dev_no == 0 && channel_no == 0) {	// 跨过裸盘hd60M.img 
				dev_no++;
				continue;
			}
			struct disk* hd = &channels[channel_no].devices[dev_no];
			struct partition* part = hd->prim_parts;
			while (part_idx < 12) {		// 4 个主分区+8个逻辑分区
				if (part_idx == 4) 
					part = hd->logic_parts;
				/* channels 数组是全局变量，默认值为0，disk属于其嵌套结构， 
     		 	 * partition 又为disk的嵌套结构，因此partition中的成员默认也为0。 
      			 * 若 partition 未初始化，则partition中的成员仍为0。 
      			 * 下面处理存在的分区 */
